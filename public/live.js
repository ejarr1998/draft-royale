...
socket.on('rejoinState', (data) => {
  ...
  if (data.players) {
    const me = data.players.find(p => p.id === mySessionId);
    console.log('👤 My player data:', {
      id: me?.id,
      name: me?.name,
      hasRoster: !!me?.roster,
      rosterLength: me?.roster?.length || 0,
      totalScore: me?.totalScore
    });
    if (me?.roster) {
      console.log('📋 Roster:', me.roster);
    }
  }
...
function renderMyRoster(player) {
  ...
  // Check if roster exists and has players
  if (!player.roster || player.roster.length === 0) {
    console.warn('⚠️ No roster data for player:', player);
    body.innerHTML = '<div class="roster-empty">No players in roster</div>';
    return;
  }
...
function goHome() {
  ...
  // Add query parameter to prevent auto-rejoin
  window.location = '/?skipRejoin=1';
}
